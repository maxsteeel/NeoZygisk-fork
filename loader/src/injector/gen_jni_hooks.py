...
class JNIHook(Method):
    ...
    def orig_method(self, name, i):
        return f'reinterpret_cast<{self.ret.type.cpp}(*)(JNIEnv *env, jclass clazz, {self.cpp()})>(get_defs()->{name}_methods[{i}].fnPtr)'
...
def gen_jni_def(name, methods):
    decl = ''
    decl += ind(0) + f'JNINativeMethod {name}_methods[{len(methods)}] = {{'
...
with open('jni_hooks.hpp', 'w') as f:
    ...
    f.write('JniHookDefinitions *get_defs();\n\n')
    f.write('struct JniHookDefinitions {\n')
